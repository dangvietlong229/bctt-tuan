...
async function main() {
  ...
  const url = "https://market.fiintrade.vn/HeatMap/GetHeatMap?ComGroupCode=VNINDEX&Type=Sector&language=vi";
  try {
    const res = await client.fetch(url, { headers: u0Headers });
    if (res.ok) {
      const data = await res.json();
      console.log("GetHeatMap keys:", Object.keys(data));
      if (data.items) {
        console.log("Count:", data.items.length);
        console.log("Sample item:", JSON.stringify(data.items[0], null, 2));
      } else {
        console.log("Sample data:", JSON.stringify(data, null, 2).slice(0, 800));
      }
    }
  } catch (e) {
    console.error(e);
