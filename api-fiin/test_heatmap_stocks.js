...
async function main() {
  ...
  const url = "https://market.fiintrade.vn/HeatMap/GetHeatMap?ComGroupCode=VNINDEX&Type=Stock&language=vi";
  try {
    const res = await client.fetch(url, { headers: u0Headers });
    if (res.ok) {
      const data = await res.json();
      console.log("GetHeatMap Stock keys:", Object.keys(data));
      const items = data.items || data;
      if (Array.isArray(items)) {
        console.log("Count:", items.length);
        if (items.length > 0) {
          console.log("Sample Stock:", JSON.stringify(items[0], null, 2));
        }
      } else {
